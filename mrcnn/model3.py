...
def initialize(mode_option, model_dir, config):
    global mode
    global model_directory
    global configuration
    global keras_model

    assert mode_option in ['training', 'inference']
    mode = mode_option
    model_directory = model_dir
    configuration = config
    set_log_dir()
    keras_model = build(mode=mode_option, config=config)
...
def build(mode, config):
    ...
    # Anchors
    if mode == "training":
        anchors = get_anchors(configuration, config.IMAGE_SHAPE)
    ...
    if mode == "training":
        ...
        model = KM.Model([input_image, processed_input_image, input_image_meta, input_anchors],
...
def set_log_dir(model_path=None):
    ...
    global log_dir
    log_dir = os.path.join(model_directory, "{}{:%Y%m%dT%H%M}".format(
        configuration.NAME.lower(), now))
...
def mold_inputs(images):
    ...
    for image in images:
        ...
        molded_image, window, scale, padding, crop = utils.resize_image(
            image,
            min_dim=configuration.IMAGE_MIN_DIM,
            min_scale=configuration.IMAGE_MIN_SCALE,
            max_dim=configuration.IMAGE_MAX_DIM,
            mode=configuration.IMAGE_RESIZE_MODE)
        molded_image = mold_image(molded_image, configuration)
        # Build image_meta
        image_meta = compose_image_meta(
            0, image.shape, molded_image.shape, window, scale,
            np.zeros([configuration.NUM_CLASSES], dtype=np.int32))
        # Append
...
def detect(images, processed_input_image, verbose=0):
    ...
    assert len(
        images) == configuration.BATCH_SIZE, "len(images) must be equal to BATCH_SIZE"
    ...
    # Anchors
    anchors = get_anchors(configuration, image_shape)
    # Duplicate across the batch dimension because Keras requires it
    # TODO: can this be optimized to avoid duplicating the anchors?
    anchors = np.broadcast_to(anchors, (configuration.BATCH_SIZE,) + anchors.shape)
    ...
    detections, _, _, mrcnn_mask, _, _, _ =\
        keras_model.predict([molded_images, processed_input_image, image_metas, anchors], verbose=0)
    # Process detections
    results = []
...
def get_anchors(configuration, image_shape):
        """Returns anchor pyramid for the given image size."""
        backbone_shapes = compute_backbone_shapes(configuration, image_shape)
        # Cache anchors and reuse if image shape is the same
        if '_anchor_cache' not in globals():
            global _anchor_cache
            _anchor_cache = {}
        if not tuple(image_shape) in _anchor_cache:
            # Generate Anchors
            a = utils.generate_pyramid_anchors(
                configuration.RPN_ANCHOR_SCALES,
                configuration.RPN_ANCHOR_RATIOS,
                backbone_shapes,
                configuration.BACKBONE_STRIDES,
                configuration.RPN_ANCHOR_STRIDE)
